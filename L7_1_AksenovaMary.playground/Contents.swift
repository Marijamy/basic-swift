...
class SweetBox{
    ...
    var Sweets = [
        "Mars" : Product(brand: "Mars", count: 3),
        "Oreo" : Product(brand: "Oreo", count: 0),
        "BonAqua" : Product(brand: "BonAqua", count: 5)
    ]
    
    var countMin = 0
    var errorValue = 100
    
    func sweetBay(itemBrand name:String, error: Int) throws {
...
func bay(collection: String, funcToBay: SweetBox) throws {
    
    let serverMessage = 100
    let collectionBar = collectionSweets[collection] ?? "Выберете товар"
    
    try funcToBay.sweetBay(itemBrand: collectionBar, error: serverMessage)
}


do {
    try bay(collection: "Water", funcToBay: SweetBox())
